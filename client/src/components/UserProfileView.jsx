...
export default function UserProfileView({ 
  ...
  const [activeTab, setActiveTab] = useState('profile'); // 'profile' | 'payments' | 'orders'
  ...
  const fetchUserOrders = async () => {
    ...
    try {
      let orderNumbers = [];
      try {
        const localSaved = localStorage.getItem('tw_my_orders');
        if (localSaved) {
          const parsed = JSON.parse(localSaved);
          if (Array.isArray(parsed)) {
            orderNumbers = parsed.map(o => o.orderNumber || o.id).filter(Boolean);
          }
        }
      } catch (e) {}
      ...
      if (orderNumbers.length > 0) params.append('orderNumbers', orderNumbers.join(','));

      const res = await fetch(`/api/orders/history?${params.toString()}`);
      const data = await res.json();
      if (data.success) {
        let fetched = data.orders || [];
        try {
          const localSaved = localStorage.getItem('tw_my_orders');
          if (localSaved) {
            const parsed = JSON.parse(localSaved);
            if (Array.isArray(parsed)) {
              const fetchedKeys = new Set(fetched.map(o => o.orderNumber || o.id));
              const missing = parsed.filter(lo => !fetchedKeys.has(lo.orderNumber) && !fetchedKeys.has(lo.id)).map(lo => {
              ...
              fetched = [...fetched, ...missing].sort((a, b) => new Date(b.createdAt) - new Date(a.createdAt));
            }
          }
        } catch (e) {}
        setOrders(fetched);
      ...
      console.error('Failed to fetch user orders in profile:', err);
    } finally {
  ...
  return (
    <div className="min-h-screen bg-[#0d2238] font-['Prompt',sans-serif] text-slate-800 pb-16">
      ...
      <div className="bg-[#0b1c2e] border-b border-[#1c334d] px-4 sm:px-8 pt-6 pb-0">
        <div className="max-w-6xl mx-auto flex flex-wrap items-center gap-2">
          ...
          <button
            ...
            className={`px-6 py-2.5 rounded-t-xl font-bold text-sm transition-all cursor-pointer font-['Kanit'] flex items-center gap-1.5 ${
              activeTab === 'orders'
                ? 'bg-[#1976d2] text-white shadow-md'
                : 'bg-white text-slate-700 hover:bg-slate-100'
            }`}
          >
            ...
            {orders.length > 0 && (
              <span className={`text-[10px] px-1.5 py-0.5 rounded-full font-bold ${
                activeTab === 'orders' ? 'bg-white text-[#1976d2]' : 'bg-blue-100 text-blue-700'
              }`}>
                {orders.length}
              </span>
            )}
      ...
      <div className="max-w-6xl mx-auto px-4 sm:px-8 pt-8">
        ...
        {/* TAB 3: ประวัติการสั่งซื้อ (Customer Order History) */}
        {activeTab === 'orders' && (
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-3xl p-6 sm:p-8 shadow-xl border border-slate-200">
              <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-slate-100 pb-5">
                <div>
                  <div className="flex items-center gap-2">
                    <h2 className="text-xl sm:text-2xl font-black text-slate-800 font-['Kanit']">
                  ...
                  <p className="text-xs text-slate-500 mt-1">
                    รายการคำสั่งซื้อ เติมเกม และการชำระเงินทั้งหมดของคุณในระบบ
                  </p>
                </div>
                ...
                <div className="flex items-center gap-2">
                  <button
                    ...
                    className="px-4 py-2 rounded-xl bg-slate-100 hover:bg-slate-200 active:scale-95 text-slate-700 font-bold text-xs transition-all flex items-center gap-1.5 cursor-pointer disabled:opacity-50"
                  >
                    {isLoadingOrders ? <Loader2 className="w-3.5 h-3.5 animate-spin" /> : <Clock className="w-3.5 h-3.5" />}
                  ...
                  {onGoToOrderHistory && (
                    <button
                      ...
                      className="px-4 py-2 rounded-xl bg-[#1976d2] hover:bg-[#1565c0] active:scale-95 text-white font-bold text-xs transition-all flex items-center gap-1.5 cursor-pointer shadow-md font-['Kanit']"
                    >
                      <span>ดูแบบละเอียด / ตัวกรอง</span>
              ...
              <div className="mt-6">
                {isLoadingOrders ? (
                  <div className="py-20 text-center">
                    ...
                    <p className="text-xs font-semibold text-slate-500">กำลังโหลดประวัติการสั่งซื้อ...</p>
                  </div>
                ) : orders.length > 0 ? (
                  <div className="overflow-x-auto rounded-2xl border border-slate-200">
                    <table className="w-full text-left text-xs border-collapse">
                      <thead>
                        <tr className="bg-slate-50/80 border-b border-slate-200 text-slate-500 font-semibold font-['Kanit'] text-[11px] uppercase tracking-wider">
                          <th className="py-3 px-4">เลขออเดอร์</th>
                          <th className="py-3 px-4">รายการ / แพ็กเกจ</th>
                          <th className="py-3 px-4">UID ผู้เล่น</th>
                          <th className="py-3 px-4">วัน-เวลาที่สั่ง</th>
                      ...
                      <tbody className="divide-y divide-slate-100">
                        {orders.map((item) => (
                          <tr key={item.id || item.orderNumber} className="hover:bg-slate-50/80 transition-colors">
                            <td className="py-3.5 px-4 font-mono font-bold text-slate-800">
                              <div className="flex items-center gap-1.5">
                                <span>{item.orderNumber || item.id}</span>
                                <button
                                  type="button"
                                  onClick={() => handleCopyOrderId(item.orderNumber || item.id)}
                                  className="text-slate-400 hover:text-slate-700 p-0.5 cursor-pointer"
                                  title="คัดลอกเลขออเดอร์"
                                >
                                  {copiedOrderId === (item.orderNumber || item.id) ? (
                            ...
                            <td className="py-3.5 px-4 font-semibold text-slate-900">
                              ...
                              <div className="text-[11px] text-slate-500 font-normal">{item.packageName}</div>
                            </td>
                            ...
                            <td className="py-3.5 px-4 font-bold text-slate-900 text-right font-['Kanit']">
                              ฿{Number(item.price || item.finalAmount || 0).toFixed(2)}
                            </td>
                            <td className="py-3.5 px-4 text-center">
                              <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-[11px] font-bold ${
                                ...
                                {item.statusLabel || (item.status === 'completed' ? 'สำเร็จ 100%' : 'รอชำระเงิน')}
                              </span>
                            </td>
                            <td className="py-3.5 px-4 text-center">
                              <button
                                ...
                                className="px-3 py-1 rounded-lg bg-slate-900 hover:bg-blue-600 text-white text-[11px] font-bold transition-all shadow-sm flex items-center justify-center gap-1 mx-auto cursor-pointer"
                  ...
                  <div className="py-16 text-center">
                    <div className="w-16 h-16 rounded-2xl flex items-center justify-center text-slate-300 mx-auto mb-3">
                    ...
                    <p className="text-xs text-slate-400 mt-1 max-w-sm mx-auto">
                      เมื่อคุณทำรายการสั่งซื้อหรือเติมเกม รายการทั้งหมดจะแสดงอยู่ที่นี่โดยอัตโนมัติ
                    </p>
                  </div>
                )}
              </div>
